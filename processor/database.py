"""
Synchronous database connection for the processor.
The worker runs in a long loop, so sync psycopg2 is simpler here.
"""
import os
import psycopg2
...
def update_video_status(video_id: str, status: str, duration: int = None):
    """Update a video's status in the database."""
    conn = get_connection()
    try:
